...
export default function QuizPage({ params }) {
    ...
    useEffect(() => {
        ...
        if (user?.id && quizId) fetchQuestions();
    }, [quizId, user, getToken]);
    ...
    const handleNext = async () => {
        ...
        try {
            const token = await getToken();
            await fetch(`${process.env.NEXT_PUBLIC_API_URL}/submitAnswer`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const handleSubmit = async () => {
        try {
            const token = await getToken();
            await fetch(`${process.env.NEXT_PUBLIC_API_URL}/completeQuiz`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            router.push(`/mocktest/test/${quizId}/feedback`);
        } catch (err) {
